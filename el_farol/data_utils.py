...
class DataUtils:
    """
    Utility class for handling data operations in the El Farol simulation.
    """
    ...
    def read_data(memories, predictors, num_agents, num_rounds, std_thresholds=[None], mirrors=True, verbose=True, many=False, tail=False):
        ...
        """
        names_ = ['Memory', 'Num_predictors', 'Identifier', 'Round', 'Agent', 'Decision', 'Score', 'Policy', 'Prediction', 'Accuracy', 'Num_agents']
        df_list = []
        for d in memories:
            for k in predictors:
                for N in num_agents:
        ...
        try:
            ...
            data['Inaccuracy'] = data['Inaccuracy'].astype(float)
        ...
        # Set column order
        if 'Std_threshold' in data.columns:
            columns = ['Memory', 'Num_predictors', 'Std_threshold', 'Num_agents', 'Num_rounds', 'Identifier', 'Round', 'Agent', 'Decision', 'Score', 'Policy', 'Prediction', 'Inaccuracy']
        else:
            columns = ['Memory', 'Num_predictors', 'Num_agents', 'Num_rounds', 'Identifier', 'Round', 'Agent', 'Decision', 'Score', 'Policy', 'Prediction', 'Inaccuracy']
        data = data[columns]
        if verbose:
    ...
    def merge_models(df):
        ...
        df['Attendance'] = df.groupby(['Model', 'Identifier', 'Round'])['Decision'].transform('mean')
        ...
        try:
            ...
            #         'Model': mod
            #     }))
        df2 = pd.concat(data_s, ignore_index=True)
        df2 = pd.merge(df2, m_attendance, on=['Model', 'Identifier'])
        df2 = pd.merge(df2, sd_attendance, on=['Model', 'Identifier'])
        df2 = df2[['Model', 'Identifier', 'Efficiency', 'Inaccuracy', 'Attendance', 'Deviation']]
        return df2
    ...
    def merge_parameters(df, parameters, variables):
        ...
        if ('Attendance' in variables) or ('Deviation' in variables):
            df['Attendance'] = df.groupby(parameters + ['Identifier', 'Round'])['Decision'].transform('mean')
...
def IQR(x):
    """
    Returns the absolute difference between the two elements of x
    with the two highest frequencies.
    """
...
def Commonality(x):
    """
    Returns the proportion of the most common value in x.
    """
    if len(x) == 0:
        return np.nan
    elif len(x) < 2:
        ...
        total = counts.sum()
        commonality = most_common / total if total > 0 else np.nan
    return (commonality - 0.5) * 2 