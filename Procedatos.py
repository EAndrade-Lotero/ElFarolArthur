...
def leer_datos(memorias, predictores, num_agentes, num_rondas, std_umbrales=[None], espejos=True, verb=True, muchos=False, cola=False):
    names_ = ['Memoria', 'Num_predic', 'Identificador', 'Ronda', 'Agente', 'Estado', 'Puntaje', 'Politica', 'Prediccion', 'Precision', 'Num_agentes']
    df_list = []       
    for d in memorias:
                ...
                for T in num_rondas:
                    for s in std_umbrales:
                        if s is None:
                            ...
                            names = names_[:2] + ['Std_umbral'] + names_[2:]
                            if verb:
                                print(f"Leyendo datos sweep memoria {d} predictores {k} desviación umbral {s} número de agentes {N} y número de rondas {T}")
                            if not muchos:
                                if espejos:
                                    archivo = './Data_Farol_Gaussian_Threshold/normal/data_todo/data_sin_espejos/simulacion-' + str(d) + "-" + str(k) + "-" + str(s) + '-' + str(N) + '-' + str(T) + ".csv"
                                else:
                                    archivo = './Data_Farol_Gaussian_Threshold/normal/data_todo/simulacion-' + str(d) + "-" + str(k) + "-" + str(s) + '-' + str(N) + '-' + str(T) + ".csv"
                            else:
                                if espejos:
                                    archivo = './Data_Farol_Gaussian_Threshold/data_todo/data_sin_espejos/simulacion-' + str(d) + "-" + str(k) + "-" + str(s) + '-' + str(N) + '-' + str(T) + ".csv"
                                else:
                                    archivo = './Data_Farol_Gaussian_Threshold/data_todo/simulacion-' + str(d) + "-" + str(k) + "-" + str(s) + '-' + '-' + str(N) + '-' + str(T) + ".csv"
                            if verb:
                                print(f"Cargando datos de archivo {archivo}...")
                            try:
                                aux = pd.read_csv(archivo, names=names, header=None)
    ...
    try:
        ...
        if 'Std_umbral' in data.columns:
            data['Std_umbral'] = data['Std_umbral'].astype(float)
    except:
        data = data.iloc[1:]
        if verb:
            print(data.head())
        ...
        if 'Std_umbral' in data.columns:
            data['Std_umbral'] = data['Std_umbral'].astype(float)
    if 'Std_umbral' in data.columns:
        columnas = ['Memoria','Num_predic','Std_umbral','Num_agentes','Num_rondas','Identificador','Ronda','Agente','Estado','Puntaje','Politica', 'Prediccion', 'Precision']
    else:
        columnas = ['Memoria','Num_predic','Num_agentes','Num_rondas','Identificador','Ronda','Agente','Estado','Puntaje','Politica', 'Prediccion', 'Precision']
    data = data[columnas]
    if verb:
        ...
        if 'Std_umbral' in data.columns:
            print("Std_umbral value counts:", data['Std_umbral'].value_counts())
